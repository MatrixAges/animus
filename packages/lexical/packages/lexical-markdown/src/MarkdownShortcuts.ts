...
function runTextMatchTransformers(
	...
	const allText = anchorNode.getTextContent()
	const lastChar = allText[anchorOffset - 1]
	...
	for (const transformer of transformers) {
		...
		if (transformer.useAllText) {
			let matchResult

			while ((matchResult = transformer.regExp.exec(allText)) !== null) {
				if (matchResult.index + 1 === anchorOffset) {
					match = matchResult
				}
			}

			if (match) {
				if (type && type !== 'add' && changeText !== transformer.trigger) {
					continue
				}

				transformer.replace(anchorNode, match, anchorOffset)
				continue
			} else {
				continue
			}
...
export function registerMarkdownShortcuts(
	...
	const $transform = (
		...
		if (runTextMatchTransformers(anchorNode, anchorOffset, textMatchTransformersIndex, type, changeText)) {
			return
		}
	...
	return editor.registerUpdateListener(({ tags, dirtyLeaves, editorState, prevEditorState }) => {
		...
		if (tags.has('collaboration') || tags.has('undo') || tags.has('redo')) {
			return
		}
		...
		const prevAnchorNode = prevEditorState._nodeMap.get(anchorKey)
		...
		editor.update(() => {
			...
			if (
				...
				anchorNode.__type === 'code_text' ||
				parentNode.__type === 'code'
			) {
				return
			}
			...
			if (prevAnchorNode) {
				const prev_text = prevAnchorNode.getTextContent()
				const anchorOffset = selection.anchor.offset
				const prevAnchoroffset = prevSelection.anchor.offset

				type = prevAnchoroffset < anchorOffset ? 'add' : 'remove'
				const sliceArgs =
					type === 'add' ? [prevAnchoroffset, anchorOffset] : [anchorOffset, prevAnchoroffset]
				change_text = prev_text.slice(...sliceArgs)
			}

			$transform(parentNode, anchorNode, anchorOffset, type, change_text)
		})
	})
}
