...
export const makeNewIdForNodeJSON = (node: BaseSerializedNode) => {
	node.key = id()

	if (node?.children?.length) {
		node.children.forEach(item => makeNewIdForNodeJSON(item))
	}
}
...
export function isDocumentFragment(x: Node | EventTarget): x is DocumentFragment {
