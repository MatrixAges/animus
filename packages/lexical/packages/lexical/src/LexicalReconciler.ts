...
import { $isDecoratorNode, $isElementNode, $isLineBreakNode, $isParagraphNode, $isRootNode, $isTextNode } from './'
import {
	DOUBLE_LINE_BREAK,
	FULL_RECONCILE,
	IS_ALIGN_CENTER,
...
function reconcileElementTerminatingLineBreak(
	...
	const prevLineBreak =
		...
		(prevElement.__size === 0 ||
			isLastChildLineBreakOrDecorator(prevElement.__last as NodeKey, activePrevNodeMap))
	const nextLineBreak =
		nextElement.__size === 0 ||
		isLastChildLineBreakOrDecorator(nextElement.__last as NodeKey, activeNextNodeMap)
