...
export const $getMatchingParent = (node: LexicalNode, findFn: (n: LexicalNode) => boolean) => {
...
export const $isSelectionInCode = (selection?: BaseSelection) => {
	...
	if (anchor_node.is(focus_node) && anchor_node.__type === 'code') return true

	const anchor_parent = anchor_node.getParent()!

	return anchor_parent.__type === 'code' && anchor_parent.is(focus_node.getParent())
}
 ...
 */
export function $getNextRightPreorderNode(startingNode: LexicalNode): LexicalNode | null {
