...
function removeParagraphsAndDivs(anchor: HTMLAnchorElement) {
	const traverse = (node: HTMLElement) => {
		for (let i = 0; i < node.childNodes.length; i++) {
			const child = node.childNodes[i] as HTMLElement

			if (child.nodeType === Node.ELEMENT_NODE) {
				if (child.tagName.toLowerCase() === 'p' || child.tagName.toLowerCase() === 'div') {
	...
	for (let i = 0; i < elements.length; i++) {
		...
		if (!IGNORE_TAGS.has(element.nodeName)) {
			if (element.nodeName === 'A') {
				removeParagraphsAndDivs(element as HTMLAnchorElement)
			}
