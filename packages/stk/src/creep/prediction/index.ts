...
export function renderPrediction({ decryptionData, crowdBlendingScore, bot = false }) {
	const {
		jsRuntime,
		jsEngine,
		htmlVersion,
	...
	const getTemplate = ({ title, agent, showVersion = false }) => {
		...
		const renderIfKnown = (decrypted: string) =>
			UNKNOWN ? ` ${HTMLNote.UNKNOWN}` : `<span class="user-agent">${decrypted}</span>`
		const renderFailingScore = (title, score) => {
			return !score || score > 36 ? title : `<span class="high-entropy">${title}</span>`
		}

		const helpTitle: string = device && gpuBrand ? [gpuBrand, device].join(':') : device ? device : gpuBrand
	...
	const deviceName = getRFPWindowOS(deviceCollection) || getBaseDeviceName(deviceCollection)
	...
	<div class="flex-grid relative">
		...
		${
			bot
				...
				: typeof crowdBlendingScore == 'number'
					? `<span class="time">crowd-blending score: ${'' + crowdBlendingScore}% <span class="scale-up grade-${crowdBlendingScoreGrade}">${crowdBlendingScoreGrade}</span></span>`
					: ''
		}
		<div class="col-six">
			...
			<div class="ellipsis relative"><span id="window-entropy"></span>${getTemplate({
				title: 'self',
				agent: windowVersion,
				showVersion: true
			})}</div>
			<div class="ellipsis relative"><span id="style-entropy"></span>${getTemplate({
				title: 'system styles',
				agent: styleSystem
			})}</div>
			<div class="ellipsis relative"><span id="styleVersion-entropy"></span>${getTemplate({
				title: 'computed styles',
				agent: styleVersion
			})}</div>
			<div class="ellipsis relative"><span id="html-entropy"></span>${getTemplate({
				title: 'html element',
				agent: htmlVersion
			})}</div>
			<div class="ellipsis relative"><span id="math-entropy"></span>${getTemplate({
				title: 'js runtime',
				agent: jsRuntime
			})}</div>
			<div class="ellipsis relative"><span id="error-entropy"></span>${getTemplate({
				title: 'js engine',
				agent: jsEngine
			})}</div>
			<div class="ellipsis relative"><span id="emoji-entropy"></span>${
				!hasValue(emojiSystem)
		...
		<div class="col-six icon-prediction-container">
			${[...iconSet]
				...
				.join('')}
			${gpuName ? `<div class="icon-prediction block-text-borderless">gpu:<br>${gpuName}</div>` : ''}
		</div>
	</div>
	`
	)
