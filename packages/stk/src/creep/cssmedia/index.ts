...
export default function getCSSMedia() {
	...
	try {
		...
		const matchMediaCSS = {
			['prefers-reduced-motion']: win.matchMedia('(prefers-reduced-motion: no-preference)').matches
				? 'no-preference'
				: win.matchMedia('(prefers-reduced-motion: reduce)').matches
					? 'reduce'
					: undefined,
			['prefers-color-scheme']: win.matchMedia('(prefers-color-scheme: light)').matches
				? 'light'
			...
			pointer: win.matchMedia('(pointer: fine)').matches
				...
				: win.matchMedia('(pointer: coarse)').matches
					...
					: win.matchMedia('(pointer: none)').matches
						? 'none'
						: undefined,
			['device-aspect-ratio']: win.matchMedia(`(device-aspect-ratio: ${deviceAspectRatio})`).matches
				? deviceAspectRatio
				: undefined,
			['device-screen']: win.matchMedia(`(device-width: ${width}px) and (device-height: ${height}px)`)
				.matches
				? `${width} x ${height}`
				: undefined,
