...
async function cipher(data: any): Promise<string[]> {
	...
	const message = btoa(String.fromCharCode.apply(null, new Uint8Array(ciphertext) as unknown as number[]))
	const vector = btoa(String.fromCharCode.apply(null, iv as unknown as number[]))
...
const getBotHash = (fp, imports) => {
	...
	if (fp.workerScope && fp.fonts) {
		...
		const windowsPlatformVersionLie =
			windowsRelease && fontPlatformVersion && !('' + windowsRelease).includes(fontPlatformVersion)
		// use font platform (window scope) to detect userAgent (worker scope) lies
		const macOrWindowsPlatformVersionLie =
			/macOS|Windows/.test(fontPlatformVersion) && platform && !fontPlatformVersion.includes(platform)
		liedPlatformVersion = windowsPlatformVersionLie || macOrWindowsPlatformVersionLie
	}
